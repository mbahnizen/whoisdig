...
$baseStorageDir = \WhoisDig\Utils\SystemHelper::normalizePath($baseStorageDir);
...
function checkRateLimit($identifier, $maxRequests = 120, $timeWindow = 3600)
{
    ...
    // Atomic read-modify-write with exclusive lock using cross-platform SystemHelper
    $fp = \WhoisDig\Utils\SystemHelper::acquireLock($file, 'c+', 5000);

    if (!$fp) {
        // Can't acquire file handle after timeout — allow request to proceed (fail-open)
